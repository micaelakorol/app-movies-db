...
const {data,error,loading} = useFetch(`https://api.themoviedb.org/3/tv/on_the_air?api_key=${api_key}&language=en-US&page=1`)
  ...
  <div className='carrusel' key={index}>
    ...
    <section className='info-first'>
      <p className='title-first'>{movie.original_name}</p>
        <Rating name='size-small' size='small' defaultValue={2}  precision={0.5} readOnly value={movie.vote_average}/>
        </section>
        </div> 
  ))}
  </div>
